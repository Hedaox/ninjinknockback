			...
			try {
				...
				if (event.source.damageType == "mob" || event.source.damageType == "player" || event.source.isProjectile()) {
					...
					// If an entity attacks a DBC player
						...
						} else if (event.source.getSourceOfDamage() instanceof EntityLiving) {
							ammount = (float) ((EntityLiving) event.source.getSourceOfDamage()).getEntityAttribute(SharedMonsterAttributes.attackDamage).getAttributeValue();
						} else {
							ammount = event.ammount;
						}

						damageA = ammount - passDefPlayerR;
